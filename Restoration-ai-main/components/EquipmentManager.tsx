...
import { Wind, Power, Clock, LayoutGrid, List, Calculator, ChevronRight, Plus, DollarSign } from 'lucide-react';
...
const EquipmentManager: React.FC<{isMobile?: boolean, project: Project, onUpdate?: (updates: Partial<Project>) => void}> = ({ isMobile = false, project, onUpdate }) => {
  ...
  const [statusFilter, setStatusFilter] = useState<'All' | 'Running' | 'Off'>('All');
  ...
  const handleAddEquipment = useCallback((model: string) => {
      ...
      <div>
        <div className="flex justify-between items-center mb-4">
             <div className="flex items-center space-x-3">
                 ...
                 <select 
                     value={statusFilter} 
                     onChange={(e) => setStatusFilter(e.target.value as 'All' | 'Running' | 'Off')}
                     className={`border ${isMobile ? 'bg-white border-gray-200 text-gray-700' : 'bg-slate-900 border-white/10 text-slate-300'} rounded-md text-xs px-2 py-1 outline-none`}
                 >
        ...
        <div className={viewMode === 'grid' ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4" : "space-y-4"}>
            {equipment.filter(e => e.status !== 'Removed' && (statusFilter === 'All' || e.status === statusFilter)).map((item) => (
