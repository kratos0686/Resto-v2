...
import { Project, RoomScan, WaterCategory, LossClass } from '../types';
...
const ProjectDetails: React.FC<ProjectDetailsProps> = ({ isMobile = false }) => {
    ...
    const [activeSubTab, setActiveSubTab] = useState<'overview' | 'scope' | 'drying' | 'photos' | 'forms' | 'predictive' | 'reference' | 'calculator' | 'ar_mapping' | 'tasks'>('overview');
            ...
            <div className="flex flex-col h-full bg-slate-950 text-slate-200">
                ...
                <header className="p-4 bg-slate-900 border-b border-white/5 sticky top-0 z-20">
                    ...
                    <div className="flex space-x-2 overflow-x-auto no-scrollbar pb-1">
                        {['Overview', 'Tasks', 'Drying', 'Scope', 'Photos', 'Calculator', 'AR Mapping'].map((tab) => (
                            <button 
                                key={tab}
                                onClick={() => setActiveSubTab(tab.toLowerCase().replace(' ', '_') as 'overview' | 'scope' | 'drying' | 'photos' | 'forms' | 'predictive' | 'reference' | 'calculator' | 'ar_mapping' | 'tasks')}
                        ...
                        <>
                             ...
                             <div className="bg-slate-900 p-5 rounded-2xl border border-white/5 space-y-4">
                                 ...
                                 {isEditingMetadata ? (
                                     ...
                                     <>
                                         <p className="text-xs text-slate-400 leading-relaxed">{project.summary || "No summary provided."}</p>
    ...
    return (
        <div className="flex h-full bg-slate-950">
            ...
            <aside className="w-64 border-r border-white/5 bg-slate-950 flex flex-col p-4 space-y-2">
                ...
                <NavButton label="Tasks" icon={<ListChecks size={18} />} active={activeSubTab === 'tasks'} onClick={() => setActiveSubTab('tasks')} />
                ...
                <NavButton label="Calculator" icon={<Calculator size={18} />} active={activeSubTab === 'calculator'} onClick={() => setActiveSubTab('calculator')} />
                                 ...
                                 {isEditingMetadata ? (
                                     <div className="space-y-4">
                                         ...
                                         <div className="grid grid-cols-2 gap-4">
                                             ...
                                             <div>
                                                 <label className="text-[10px] font-bold uppercase text-slate-500 block mb-1">Adjuster</label>
                                                 <input 
                                                     className="w-full bg-slate-900 border border-white/10 rounded-lg p-2 text-xs text-slate-200"
                                     ...
                                     <div className="space-y-4">
                                         <div>
                                             ...
                                             <p className="text-xs text-slate-400 leading-relaxed bg-slate-900/50 p-3 rounded-xl border border-white/5">
                                                 {project.summary || "No loss summary available for this project."}
                                             </p>
                                         </div>
