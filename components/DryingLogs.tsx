      ...
      <div className="space-y-6 pb-24">
        ...
        <div className="bg-slate-900 border border-white/10 rounded-[2rem] p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Moisture Content Goals</h3>
            <span className="text-[10px] bg-white/10 px-2 py-1 rounded text-slate-300 font-medium">Auto-saves on blur</span>
          </div>

          <div className="space-y-6">
            {Object.keys(materialsByLocation).length === 0 ? (
              <div className="text-center py-8 text-slate-500 text-sm bg-black/20 rounded-xl">No materials tracked yet. Switch to wizard to add.</div>
            ) : (
               Object.entries(materialsByLocation).map(([location, mats]) => (
                 <div key={location} className="space-y-3">
                   <h4 className="text-sm font-bold text-white border-b border-white/10 pb-2">{location}</h4>
                   <div className="grid gap-3">
                     {mats.map((mat) => {
                       const currentReading = mat.readings?.length > 0 ? mat.readings[mat.readings.length - 1].value : mat.initialReading;
                       return (
                         <div key={mat.id} className="bg-black/40 border border-white/5 rounded-xl p-3 flex flex-col md:flex-row md:items-center justify-between gap-3">
                           <div className="flex flex-col">
                             <span className="text-sm font-bold text-slate-200">{mat.name}</span>
                             <span className="text-[10px] text-slate-500 uppercase tracking-widest flex items-center space-x-2">
                               Status: <span className={mat.status === 'Dry' ? 'text-emerald-400 ml-1' : 'text-yellow-400 ml-1'}>{mat.status}</span>
                             </span>
                           </div>
                           
                           <div className="flex items-center gap-4">
                             <div className="flex flex-col">
                               <label className="text-[10px] text-slate-500 uppercase font-bold">Current (%)</label>
                               <span className={`text-sm font-mono font-bold ${mat.status === 'Dry' ? 'text-emerald-400' : 'text-yellow-400'}`}>
                                 {currentReading !== undefined ? currentReading : '--'}
                               </span>
                             </div>
                             <div className="flex flex-col">
                               <label className="text-[10px] text-slate-500 uppercase font-bold">Goal (%)</label>
                               <input 
                                 type="number"
                                 defaultValue={mat.dryGoal}
                                 className="w-16 bg-slate-800 border border-white/20 rounded px-2 py-1 text-sm font-mono text-white outline-none focus:border-brand-cyan focus:ring-1 focus:ring-brand-cyan"
                                 onBlur={(e) => {
                                   const newGoal = parseFloat(e.target.value);
                                   if (!isNaN(newGoal) && onUpdate) {
                                     // Update this specific material's goal
                                     const updatedMaterials = trackedMaterials.map(m => {
                                       if (m.id === mat.id) {
                                         const current = m.readings?.length > 0 ? m.readings[m.readings.length - 1].value : m.initialReading;
                                         return {
                                           ...m,
                                           dryGoal: newGoal,
                                           status: (current <= newGoal) ? 'Dry' : 'Wet'
                                         };
                                       }
                                       return m;
                                     });
                                     onUpdate({ dryingMonitor: updatedMaterials });
                                   }
                                 }}
                               />
                             </div>
