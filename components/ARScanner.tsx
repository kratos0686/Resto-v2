...
const ARScanner: React.FC<ARScannerProps> = ({ onComplete }) => {
  ...
  useEffect(() => {
     if (autoCaptureEnabled && mode === 'scan' && trackingQuality === 'Good' && capturedImages.length < 8) {
         ...
         // Trigger if viewing angle changes significantly (reaching a "new frame")
         if (isFirst || alphaDiff > 25 || betaDiff > 25 || gammaDiff > 25) {
             lastCaptureOrientation.current = { ...orientation };
             handleCapture();
         }
     }
     // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [orientation, autoCaptureEnabled, mode, trackingQuality, capturedImages.length]);

  useEffect(() => {
     const count = capturedImages.length;
     if (count === 0) setGuidanceMessage("Point at a corner and hold steady.");
     else if (count >= 1 && count <= 2) setGuidanceMessage("Slowly pan along the walls.");
     else if (count > 2 && count < 5) setGuidanceMessage("Ensure doors and windows are visible.");
     else if (count >= 5 && count < 8) setGuidanceMessage("Capture the floor and ceiling features.");
     else setGuidanceMessage("Sufficient data. Tap to process.");
    ...
    <div className="h-full relative bg-black overflow-hidden flex flex-col">
        {mode === 'scan' && (
            <>
                ...
                {/* AI Guidance Overlay */}
                <div className="absolute top-24 left-1/2 -translate-x-1/2 w-72 p-3 bg-slate-900/80 border border-brand-cyan/30 rounded-xl backdrop-blur-md text-center shadow-lg pointer-events-none z-30">
            ...
            <div className="h-full flex flex-col bg-slate-100">
                ...
                <div className="bg-white p-6 border-t border-slate-200 space-y-4 shadow-[0_-5px_20px_rgba(0,0,0,0.05)] z-20">
                    ...
                    <div className="flex justify-between items-center p-3 bg-slate-50 rounded-xl border border-slate-200">
                        <div className="text-xs font-bold text-slate-500 uppercase">Calculated Area</div>
                        <div className="text-xl font-black text-slate-900">{aiDimensions.sqft.toFixed(0)} <span className="text-xs text-slate-400 font-medium">sq ft</span></div>
                    </div>
