...
const SmartDocumentation: React.FC<SmartDocumentationProps> = ({ project, onUpdate, onBack }) => {
    ...
    const handleApplyRecommendation = () => {
        const newLog: DailyNarrative = {
            ...
            date: new Date().toLocaleDateString(),
            timestamp: Date.now(),
            content: 'Added 1 LGR Dehumidifier to containment zone based on psychrometric recommendation.',
            author: currentUser?.name || 'Technician',
            tags: ['Equipment', 'Auto-Applied'],
            generated: true,
            entryType: 'equipment',
            data: { action: 'add', type: 'Dehumidifier', model: 'LGR' }
        };
        ...
        EventBus.publish('com.restorationai.equipment.state.changed', newLog.data, project.id, newLog.content, 'success');
                    ...
                    <div className="bg-gradient-to-r from-indigo-900/40 to-blue-900/40 border border-indigo-500/20 rounded-2xl p-4 flex items-start space-x-3 animate-in fade-in slide-in-from-top-4">
                        ...
                        <div className="flex-1">
                            ...
                            <div className="mt-2 flex space-x-2">
                                 <button onClick={handleApplyRecommendation} className="px-3 py-1 bg-indigo-600 hover:bg-indigo-500 rounded-lg text-[10px] font-bold text-white transition-colors">Apply Recommendation</button>
                                 <button onClick={() => setShowRecommendation(false)} className="px-3 py-1 bg-white/5 hover:bg-white/10 rounded-lg text-[10px] font-bold text-slate-400 transition-colors">Dismiss</button>
                            </div>
                        </div>
                    </div>
                )}
